...
def naive_softmax_loss_and_gradient(
    ...
    grad_center_vec = np.dot(y_hat, outside_vectors) - u_o

    # Gradient with respect to outside vectors
    grad_outside_vecs = np.outer(y_hat, center_word_vec)
    grad_outside_vecs -= np.outer(y, center_word_vec)
    ### END YOUR CODE
...
def neg_sampling_loss_and_gradient(
    ...
    ### YOUR CODE HERE    
    new_outside_vectors = outside_vectors[indices].copy()
    new_outside_vectors[1:] *= -1
    
    sigmoid_dot_products = sigmoid(np.dot(center_word_vec, new_outside_vectors.T))
    ...
    # Gradient with respect to outside vectors
    grad_outside_vecs_non_zeros = np.outer(1-sigmoid_dot_products, center_word_vec)
...
def skipgram(current_center_word, outside_words, word2ind,
    ...
    for outside_word in outside_words:
        ...
        cur_loss, cur_grad_center_vecs, cur_grad_outside_vectors = word2vec_loss_and_gradient(
            center_word_vec,
            outside_word_idx,
            outside_vectors,
            dataset
        ...
        loss += cur_loss
        grad_center_vecs[center_word_idx] += cur_grad_center_vecs
        grad_outside_vectors += cur_grad_outside_vectors
